...
from core.base import rollout
#from model_utils import learn_policy


def plot_databuffer(data, data_index = 2, ylim=None):
    fig, ax = plt.subplots(1, 1)
    D = data.observation_dim
    E = data.action_dim
    ...
    for i in range(len(data.buffer)):
        ...
        if data_index < D:
            s = np.concatenate([tau[:, data_index], [tau[-1, D + E + data_index]]])
        else:
            s = np.concatenate([tau[:, data_index]])
...
def plot_trajectories(env, policy, policy_optimizer, dynamics, N=50, T=25, predict_uncertainty=False, ylim=None):
    ...
    for n in range(N):
        ...
        tau_sys = rollout(env, policy, T=T, mode='System', init_particle=init_particle, render=False)
        ...
        tau_NN = rollout(env, policy, dynamics, T=T, mode='NN', init_particle=init_particle, render=False)
