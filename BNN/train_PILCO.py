...
parser.add_argument('--num_itr_dyn', type=int, default=10)  # epoch
...
parser.add_argument('--drop_p', type=float, default=0.01)
...
for i in range( num_iter_algo):
    ...
    for j in range(num_iter_policy):
        ...
        if (j + 1) % log_interval_policy == 1 or (j + 1) == args.num_iter_policy:
            ...
            loss_mean = torch.cat(list_costs) .data.cpu().numpy()[0]
